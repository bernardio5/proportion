...
const PR_L = 2;  // line
const PR_C = 3;  // circle
const PR_R = 4;  // real -- a float, stored in t1
...
// set as real
prBase.prototype.saR = function(t1in) { 
	this.t = PR_R; 
	this.t1 = t1in; 
}
...
prBase.prototype.saPm = function ( ob,  tin) {
	switch (ob.t) {
		...
		case PR_C:
			...
			break;
		case PR_R: 
			this.cl(); // return NULL if input is real
			break; 
		case PR_S: 
...
// same as saPm, but taking a PR_R object as the parameter
...
// machine-related code. 

// reserved words for machine commands
const PR_ERROR 		= 0; 	

const PR_GIVEN 		= 1; 	// // commands that create prBase
const PR_LINE 		= 2; 
const PR_CIRCLE 	= 3; 
const PR_REAL		= 4;
const PR_PARA 		= 5; 
...
const PR_SEGMENT	= 9;	// // manipulations of canvas state??
const PR_ARC		= 10;
const PR_TRACE		 = 11; 	
const PR_TRACE_GROUP = 12; 	
const PR_LOFT		 = 13; 
const PR_CLEAR		 = 14; 
const PR_PAUSE		 = 15; 
const PR_SETGROUP	 = 16; 

							// // internal labelling; not commands
const PR_OUTPUT		= 17; 	// label outputs
const PR_INPUT 		= 18; 	//  "    inputs
const PR_EOL 		= 19; 	// token to divide up commands
const PR_MACHINE 	= 20; 	// first line of machine; names machine
const PR_END 		= 21; 	// last line of machine

const PR_CMD_COUNT 	= 22; 	// how many commands are there? 



// instruction objects hold the data needed to do one operation 
// arguments are integers. positive integers are indices into prPage.objs[].
//   negative numbers are (negated) indices into the argument list. 
//       the argument list's indices begin at 1; 0 is the NULL. 

// but everything is done referring to the already-present contents of the page!
// In faaact, everything is done by referring to contents relative to the last mark. 
// so everything starts out negative and gets translated positive. right? 

// oh, who cares. just read a fucking JSON tutorial, you pretentious twat. 

function prInst() { 
	this.op = PR_ERROR; 
	this.arg1 = 0; 
	this.arg2 = 0; 
	this.arg3 = 0; // no command uses more than three arguments. 
}


prInst.prototype.copy = function(it) { 
	this.op = it.op; 
	this.arg1 = it.arg1; 
	this.arg2 = it.arg2; 
	this.arg3 = it.arg3; 
...
// machines are lists of instructions. 
// they have inputs. they are attached to pages, which hold both input and output. 
//    so, they are side-effect heavy. hope you don't mind. 

// instructions use the "command" fn of prPage to do things. 
//   each instruction specifies an operation (one of 10 or so)
...
prMachine.prototype.copy = function(it) { 
	this.name = it.name; 
	this.instCount = it.instCount; 
	var i; 
	for (i=0; i<this.instCount; ++i) { 
		this.ins[i].copy(it.ins[i]); 
	}
}
...
prPage.prototype.copy = function(it) {
	...
	this.macCount = it.macCount; 
	for (i=0; i<this.macCount; ++i) { 
		this.macs[i].copy(it.macs[i]); 
	}
...
prPage.prototype.merge = function(it) {
	...
	mx = this.macCount; 
	for (i=0; i<it.macCount; ++i) { 
		this.macs[i+mx].copy(it.macs[i]); 
	}
}
...
prPage.prototype.addReal = function(n) { 
	var res = this.objCount;
		
	this.objs[this.objCount] = new prBase;
	this.objs[this.objCount].saR(n);
	++this.objCount;
...
/// instead of parameters, take points. 
prPage.prototype.addSegmentOb = function(ob, p1, p2) { 
	var t1, t2; 
	t1 = this.closestPointParam(ob, p1); 
	t2 = this.closestPointParam(ob, p2); 
	this.addSegment(ob, t1, t2); 
}


/// instead of parameters, take points. 
prPage.prototype.addSegmentObMid = function(ob, p1, p2) { 
	var t1, t2; 
	t1 = this.closestPointParam(ob, p1); 
	t2 = this.closestPointParam(ob, p2); 
	this.addSegment(ob, t1, t2); 
...
prPage.prototype.addSegmentOb = function(ob, p1, p2) { 
	...
	t1 = this.closestPointParam(ob, p1); 
	t2 = this.closestPointParam(ob, p2); 
	this.addArc(ob, t1, t2); 
}


prPage.prototype.addSegmentOb = function(ob, p1, p2, p3) { 
	var t1, t2, t3; 
	t1 = this.closestPointParam(ob, p1); // first 
	t2 = this.closestPointParam(ob, p2); // middle 
	t3 = this.closestPointParam(ob, p3); // last
	
	/// buuuuuut, what if the arc crosses 0?  
	if ( (t1<t3) && ((t2<t1)||(t3<t2)) ) { t1 += 1.0; }
	if ( (t1>t3) && ((t1<t2)||(t2<t3)) ) { t3 += 1.0; }
	
	this.addArc(ob, t1, t3); 
}
...
prPage.prototype.addParametricPointOb = function(obj, r) { 
	var res = 0;
	if (this.objs[r].t==PR_R) { 
		res = this.addParametricPoint(obj, this.objs[r].t1);
	}
	return res;
}
...
prPage.prototype.closestPointParamOb = function(obj, pt) {
	var res = 0;
	this.objs[this.objCount] = new prBase;
	this.objs[this.objCount].saR(this.objs[obj], this.objs[obj].cPP(this.objs[pt]));
	if (this.objs[this.objCount].t!=PR_N) {
		res = this.objCount;
		++this.objCount;
	}
...
// given 
prPage.prototype.command = function(which, arg1, arg2, arg3) { 
	var res, m1, m2; 
	m1 = new prBase; 
	m2 = new prBase; 
	...
	switch (which) { 
		case PR_GIVEN:
			res = this.addGivenPointOp(arg1, arg2); // should point to objs of type PR_T!
			break; 
		case PR_LINE:
			res = this.addLine(arg1, arg2); // two points
			break; 
		case PR_CIRCLE:
			res = this.addCircle(arg1, arg2); // two points
			break;		
		case PR_PARA: 
			res = this.addParametricPointOp(arg1, arg2); // one object, one PR_T
			break;		
		case PR_FIRST:
			res = this.addFirstIntersection(arg1, arg2, arg3); // obj obj pt
			break;		
		case PR_SECOND:
			res = this.addSecondIntersection(arg1, arg2, arg3); // 
			break;		
		case PR_CLOSEST:
			res = this.addClosestPoint(arg1, arg2); // two points
			break;		
			
		case PR_SEGMENT:
			res = this.addSegmentOp(arg1, arg2, arg3); // two points
			break;		
		case PR_ARC:
			res = this.addArcOp(arg1, arg2, arg3); // two points
			break;		
		case PR_TRACE:
			res = this.trace(); // object, group ID
			break;		
		case PR_TRACE_GROUP:
			res = this.traceOb(arg1); // object, group ID
			break;		
		case PR_SETGROUP:
			res = this.setGroupOb(arg1, arg2); // object, group ID
			break;		

		case PR_LOFT:
			res = this.loft(arg1, arg2); // 2 marks
			break;		

		case PR_CLEAR:
			res = this.clear(); // object, group ID
			break;		

		case PR_PAUSE:
			res = 0; // object, group ID
			pause(this.objs[arg1].t1); 
			break;		
...
prPage.prototype.loadMachines = function(filename) { 
	// uh. gawd. 
}



prPage.prototype.run = function(macName, args, argCount) { 
	for (i=0; i<this.macCount; ++i) { 
		if (this.macs[i].name == macName) {
			ct = this.macs[i].instrCount; 
			for (j=0; j<ct; ++j) { 
