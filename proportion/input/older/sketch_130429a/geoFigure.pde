...
public class geoFigure {
  ...
  // two points only.
  public geoBase makeLine(int ob1, int ob2) {
    geoBase goalie; 
    goalie = new geoBase(); 
    ...
    if (objs[ob1].isAPoint()!=0) {
      if (objs[ob2].isAPoint()!=0) { 
        if (objs[ob1].equalPoints(objs[ob2])!=1) { 
          goalie.setAsLine(objs[ob1].x, objs[ob1].y, objs[ob2].x, objs[ob2].y);
        }
      }
    }
    return goalie;
  ...
  // two points only.
  public geoBase makeCircle(int ob1, int ob2) {
      geoBase goalie; 
    goalie = new geoBase(); 
    ...
    if (objs[ob1].isAPoint()!=0) { 
      if (objs[ob2].isAPoint()!=0) { 
        if (objs[ob1].equalPoints(objs[ob2])!=1) { 
           goalie.setAsCircle(objs[ob1].x, objs[ob1].y, objs[ob2].x, objs[ob2].y);
        }
      }
    }
    return goalie;
